...
export default function Home() {
  ...
  const [imageUnderstandings, setImageUnderstandings] = useState<string[] | null>(null);
  const [isImageLoading, setIsImageLoading] = useState(false);
  const [isReferenceLoading, setIsReferenceLoading] = useState(false);
  const [isFeedbackLoading, setIsFeedbackLoading] = useState(false);

  const handleFullPdf2Images = async () => {
    try {
      const response = await axios.post(
        "/api/pdf-to-img-rephrase",
        { 
          course: "E-Learning"
        }
      );
      setImageUnderstandings(response.data.understandings)
      console.log(response.data.understandings)
    } catch (error) {
      console.error('Error fetching rephrased information', error);
    }
  };

  // const handleMultiRetrieve = async () => {
  //   try {
  //     const response = await axios.post(
  //       "/api/multi-embed",
  //       { 
  //         understanding: imageUnderstandings,
  //         course: "E-Learning"
  //       }
  //     );
  //     setImageUnderstandings(response.data.understandings)
