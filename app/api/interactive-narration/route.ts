  ...
  try {
    ...
    if (!student_answer || !feedback || !reference_content || !slide_title) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: student_answer, feedback, reference_content, slide_title",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      "Backend interactive narration response status:",
      response.status
    );
    ...
    console.log(
      "Backend returned interactive narration with audio length:",
      data.audio_base64?.length || 0
    );
