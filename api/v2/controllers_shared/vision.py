...
def encode_image_shared(image_byte_arr):
    image_byte_arr.seek(0)
    img_base64 = base64.b64encode(image_byte_arr.read()).decode('utf-8')
    return img_base64

def vision_controller_shared(image_path, settings: Annotated[Settings, Depends(get_settings)], print_stream=False):
    ...
    base64_image = encode_image_shared(image_path)
...
class VisionResponseShared(BaseModel):
    slide_content: str

def set_vision_shared(img_base64: List[str], settings: Annotated[Settings, Depends(get_settings)]):
    api_key = settings.openai_api_key
    ...
    result = client.beta.chat.completions.parse(
        ...
        response_format=VisionResponseShared,
    )
